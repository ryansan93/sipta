...
<div class="modal-body body" style="padding-bottom: 0px;">
	<div class="row">
			...
			<select class="col-sm-12 form-control mahasiswa" data-required="1">
				...
				<?php if ( !empty($mahasiswa) ): ?>
					<?php foreach ($mahasiswa as $key => $value): ?>
						<?php
							$selected = null;
							if ( $value['nim'] == $data['nim'] ) {
								$selected = 'selected';
							}
						?>
						<option value="<?php echo $value['nim']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($value['nim'].' | '.$value['nama']); ?></option>
					<?php endforeach ?>
				<?php endif ?>
			</select>
		</div>
				...
				<fieldset>
					...
					<small>
						<table class="table table-bordered tbl_pembimbing" style="margin-bottom: 0px;">
							...
							<tbody>
								<?php foreach ($data['sk_pembimbing_dosen'] as $k_spd => $v_spd): ?>
									<tr>
										<td>
											<select class="col-sm-12 form-control dosen" data-required="1">
												...
												<?php if ( !empty($dosen) ): ?>
													<?php foreach ($dosen as $key => $value): ?>
														<?php
															$selected = null;
															if ( $value['nip'] == $v_spd['nip'] ) {
																$selected = 'selected';
															}
														?>
														<option value="<?php echo $value['nip']; ?>" <?php echo $selected; ?> ><?php echo strtoupper($value['nip'].' | '.$value['nama']); ?></option>
													<?php endforeach ?>
												<?php endif ?>
											</select>
										</td>
		...
		<div class="col-sm-12 no-padding" style="padding-left: 0px; padding-right: 0px;">
			<hr>
			<button type="button" class="btn btn-primary pull-right" onclick="skp.edit(this)" data-id="<?php echo $data['id']; ?>">
