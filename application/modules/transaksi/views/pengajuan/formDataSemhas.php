<?php if ( !empty($data) ): ?>
	<?php 
		$cek_seminar_sempro = 0;
		$cek_pelaksanaan_ta = true;
		if ( stristr($jenis_pengajuan, 'seminar hasil') !== false ) {
			if ( $jml_sempro < 10 ) {
				$cek_seminar_sempro = 1;
			}
			if ( $pelaksanaan_ta ) {
				$cek_pelaksanaan_ta = $pelaksanaan_ta;
			} else {
				$cek_pelaksanaan_ta = false;
			}
		}
	?>

	<?php if ( $cek_seminar_sempro == 0 && $cek_pelaksanaan_ta == true ): ?>
	...
	<?php else: ?>
		...
		<?php endif ?>
		<?php if ( $cek_pelaksanaan_ta == false ): ?>
