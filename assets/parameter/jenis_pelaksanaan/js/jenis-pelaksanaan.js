var jp = {
	...
	modalAddForm: function () {
        ...
        $.get('parameter/JenisPelaksanaan/modalAddForm',{
        },function(data){
            var _options = {
                className : 'large',
                message : data,
	...
	modalEditForm: function (elm) {
        ...
        $.get('parameter/JenisPelaksanaan/modalEditForm',{
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin menyimpan data ?', function(result) {
				if ( result ) {
			        ...
			        $.ajax({
			            url: 'parameter/JenisPelaksanaan/save',
			            data: {
			                'params': data
			            },
			            type: 'POST',
		...
		if ( err > 0 ) {
			...
			bootbox.confirm('Apakah anda yakin ingin meng-ubah data ?', function(result) {
				if ( result ) {
			        ...
			        $.ajax({
			            url: 'parameter/JenisPelaksanaan/edit',
			            data: {
			                'params': data
			            },
			            type: 'POST',
    ...
    delete: function(elm) {
		...
		bootbox.confirm('Apakah anda yakin ingin meng-hapus data ?', function(result) {
			if ( result ) {
		        ...
		        $.ajax({
		            url: 'parameter/JenisPelaksanaan/delete',
		            data: {
		                'kode': kode
		            },
		            type: 'POST',
